...
BIO_write_filename(PrivateKeyOut, PAnsiChar(fPrivateKeyFile));
PublicKeyOut := BIO_new(BIO_s_file());
BIO_write_filename(PublicKeyOut, PAnsiChar(fPublicKeyFile));
...
BIO_read_filename(p7file, PAnsiChar(Filename));
...
if BIO_write_filename(pkcs7file, PAnsiChar(Filename)) <= 0 then
...
if BIO_write_filename(contentfile, PAnsiChar(Filename)) <= 0 then
  raise EOpenSSL.Create('Error creating output file.');
...
BIO_read_filename(keyfile, PAnsiChar( ansistring(KeyFilename) ) );
if KeyPassword = '' then
  pw := nil
else
